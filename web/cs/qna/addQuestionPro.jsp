...
<%
    String path67 = request.getContextPath();
    //2. 보내온 데이터 받기
    ...
    if(cnt==2){
        ...
        response.sendRedirect(path67+"/cs/qna/qnaList.jsp?page=1");
    } else if(cnt==1){
        System.out.println("답변 글이 등록되었습니다.");
        response.sendRedirect(path67+"/cs/qna/qnaList.jsp?page=1");
    } else {
        System.out.println("질문 및 답변 등록이 실패되었습니다.");
        response.sendRedirect(path67+"/cs/qna/addQuestion.jsp?lev="+lev+"&par="+par);
